/*(function (Drupal, drupalSettings) {
  ...
  // Гарантируем, что используем тот же jQuery, к которому привязан Highcharts
  var $ = window.jQuery;

  Drupal.behaviors.budgetIncomesChart = {
    attach: function (context, settings) {*/
      ...
      if (typeof data != 'undefined' && typeof data.data != 'undefined' ) {
        ...
        console.log($.fn.highcharts === window.jQuery.fn.highcharts, '!!!');
        var $container = $('#infographics-1');//, context);
        if ($container.length === 0) {
          console.log('нет контенера');
        }
        if ($container.data('highcharts-chart')) {
          //
          console.log('График уже инициализирован');
        }

        $container.highcharts($.extend(true, optionsStackedColumn(), opts));
