...
class ExecExpensAjaxCntr extends ControllerBase {
  ...
  public function getData(Request $request) {
    ...
    $arData = [];
    if (!empty($date)) {
      $arIncomes = $data['execution']['expense_sector'][$date] ?? [];
    } else {
      $date = key($data['execution']['expense_sector']);
      $arIncomes = reset($data['execution']['expense_sector']);
    }
